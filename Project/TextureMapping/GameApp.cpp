...
bool GameApp::InitEffect() {
	...
	HR(CreateShaderFromFile("HLSL\\Basic_3D_VS.cso", L"HLSL\\Basic_3D_VS.hlsl", "VS", "vs_5_0", blob.ReleaseAndGetAddressOf()));
	...
	HR(CreateShaderFromFile("HLSL\\Basic_3D_PS.cso", "HLSL\\Basic_3D_PS.hlsl", "PS", "ps_5_0", blob.ReleaseAndGetAddressOf()));
...
bool GameApp::InitResource() {
	...
	HR(CreateDDSTextureFromFile(m_pD3D11Device.Get(), L".\\Texture\\WoodCrate.dds", nullptr, m_pWoodCrate.GetAddressOf()));

	WCHAR strFile[40];
	m_pFireAnims.resize(120);
	for(int i = 1; i <= 120; i++) {
		wsprintf(strFile, L".\\Texture\\FireAnim\\Fire%3d.bmp",i);
		HR(CreateDDSTextureFromFile(m_pD3D11Device.Get(), strFile,nullptr, m_pFireAnims[static_cast<size_t>(i) - 1].GetAddressOf()));
	}
	...
	m_PSConstantBuffer.pointLight[0].attr = XMFLOAT3(0.0f,0.1f,0.0f);
	...
	m_pD3D11DeviceContext->PSSetSampler(0,1,m_pSamplerState.GetAddressOf());
	m_pD3D11DeviceContext->PSSetShaderResource(0,1,m_pWoodCrate.GetAddressOf());
	m_pD3D11DeviceContext->PSSetShader(m_pPixelShader3D.Get(),nullptr,0);
