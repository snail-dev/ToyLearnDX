...
void BasicEffect::SetDrawBoltAnimNoDepthWriteWithStencil(ID3D11DeviceContext* device_context, UINT stencilRef){
	device_context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	device_context->IASetInputLayout(pImpl->m_pVertexLayout3D.Get());
	device_context->VSSetShader(pImpl->m_pVertexShader3D.Get(), nullptr, 0);
	device_context->RSSetState(RenderStates::RSNoCull.Get());
	device_context->PSSetShader(pImpl->m_pPixelShader3D.Get(), nullptr, 0);
	device_context->PSSetSamplers(0, 1, RenderStates::SSLinearWrap.GetAddressOf());
	device_context->OMSetDepthStencilState(RenderStates::DSSNoDepthWriteWithStencil.Get(), stencilRef);
	device_context->OMSetBlendState(RenderStates::BSAdditive.Get(), nullptr, 0xFFFFFFF);
}

void BasicEffect::SetDrawBoltAnimNoDepthWrite(ID3D11DeviceContext* device_context) {
	device_context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	device_context->IASetInputLayout(pImpl->m_pVertexLayout3D.Get());
	device_context->VSSetShader(pImpl->m_pVertexShader3D.Get(), nullptr, 0);
	device_context->RSSetState(RenderStates::RSNoCull.Get());
	device_context->PSSetShader(pImpl->m_pPixelShader3D.Get(), nullptr, 0);
	device_context->PSSetSamplers(0, 1, RenderStates::SSLinearWrap.GetAddressOf());
	device_context->OMSetDepthStencilState(RenderStates::DSSNoDepthWrite.Get(), 0);
	device_context->OMSetBlendState(RenderStates::BSAdditive.Get(), nullptr, 0xFFFFFFF);
}
