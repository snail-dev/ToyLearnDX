...
bool GameApp::Init() {
	return D3DApp::Init() && (RenderStates::InitAll(m_pD3D11Device.Get()), m_BasicEffect.InitAll(m_pD3D11Device.Get())) && InitResource();
}
void GameApp::OnResize() {
	...
	if(m_pCamera != nullptr) {
		...
		m_BasicEffect.SetProjectionMatrix(m_pCamera->GetProjXM());	}
}
...
void GameApp::UpdateScene(float dt) {
	...
	static float flash_per_frame_time = 0.0f;
	m_BoltAnim.SetTexture(m_SRVs[flash_index].Get());

	if (flash_per_frame_time > 1.0 / 60) {
		flash_per_frame_time -= 1.0 / 60;
		flash_index++;
		flash_index %= 60;
	}
	flash_per_frame_time += dt;
}
...
void GameApp::DrawScene() {
	...
	m_BasicEffect.SetWriteStencilOnly(m_pD3D11DeviceContext.Get(), 1);
	m_Mirror.Draw(m_pD3D11DeviceContext.Get(), m_BasicEffect);
	...
	m_BasicEffect.SetRenderDefaultWithStencil(m_pD3D11DeviceContext.Get(), 1);

	m_Walls[2].Draw(m_pD3D11DeviceContext.Get(), m_BasicEffect);
	m_Walls[3].Draw(m_pD3D11DeviceContext.Get(), m_BasicEffect);
	m_Walls[4].Draw(m_pD3D11DeviceContext.Get(), m_BasicEffect);
	m_Floor.Draw(m_pD3D11DeviceContext.Get(), m_BasicEffect);
	m_WoodCrate.Draw(m_pD3D11DeviceContext.Get(), m_BasicEffect);
	...
	m_BasicEffect.SetRenderNoDoubleBlend(m_pD3D11DeviceContext.Get(), 1);
	...
	m_BasicEffect.SetDrawBoltAnimNoDepthWriteWithStencil(m_pD3D11DeviceContext.Get(),1);
	m_BoltAnim.Draw(m_pD3D11DeviceContext.Get(), m_BasicEffect);

	m_BasicEffect.SetReflectionState(false);
	m_BasicEffect.SetRenderAlphaBlendWithStencil(m_pD3D11DeviceContext.Get(),1);
	m_Mirror.Draw(m_pD3D11DeviceContext.Get(),m_BasicEffect);

	// ******************
	// 5. 绘制不透明的正常物体
	//
	m_BasicEffect.SetRenderDefault(m_pD3D11DeviceContext.Get());

	for (auto& wall : m_Walls)
		wall.Draw(m_pD3D11DeviceContext.Get(), m_BasicEffect);
	m_Floor.Draw(m_pD3D11DeviceContext.Get(), m_BasicEffect);
	m_WoodCrate.Draw(m_pD3D11DeviceContext.Get(), m_BasicEffect);
	...
	m_BasicEffect.SetRenderNoDoubleBlend(m_pD3D11DeviceContext.Get(), 0);
	...
	m_BasicEffect.SetDrawBoltAnimNoDepthWrite(m_pD3D11DeviceContext.Get());
	m_BoltAnim.Draw(m_pD3D11DeviceContext.Get(), m_BasicEffect);
...
bool GameApp::InitResource() {
	...
	m_SRVs.assign(60, nullptr);
	wchar_t buf[50];
	for (int i = 1; i <= 60; i++) {
		memset(buf, 0, sizeof(buf));
		wsprintf(buf, L"Texture\\BoltAnim\\Bolt%03d.bmp", i);
		HR(CreateWICTextureFromFile(m_pD3D11Device.Get(), buf, nullptr, m_SRVs[i - 1].GetAddressOf()));
	}
	m_BoltAnim.SetBuffer(m_pD3D11Device.Get(), Geometry::CreateCylinderNoCap(4.0f, 4.0f));
	...
	m_Floor.SetBuffer(m_pD3D11Device.Get(),
	...
	m_Mirror.SetBuffer(m_pD3D11Device.Get(),
	...
	pointLight.attr = XMFLOAT3(0.0f, 0.1f, 0.0f);
